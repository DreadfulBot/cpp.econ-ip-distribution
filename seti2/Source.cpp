...
FourOctets GetMaskBySize(int size) {
	std::string temp = std::to_string(size);
	return GetMaskBySize(temp);
}
...
// ip-адрес[/]маска
bool SplitStrIpMask(std::string src, FourOctets& ipAddress, FourOctets& ipMask, int& maskLenght) {
	size_t targetSlashOffset = 0;
	size_t curSlashOffset = 0;
	size_t numOfSlashes = 0;

	// формируем ip-адрес 1
	while( (curSlashOffset = src.find( '/' )) != std::string::npos) {
		if(curSlashOffset != std::string::npos) {
			targetSlashOffset = curSlashOffset;
		}
		src.replace(curSlashOffset, 1, 1, '_');
	...
	ipAddress = GetIpAddress(src.substr(0, targetSlashOffset));

	// формируем маску
	if(ipAddress.isValid = true) {
		const char* temp = src.c_str();
		temp += targetSlashOffset + 1;
		maskLenght = atoi(temp);
		ipMask = GetMaskBySize(src.substr(targetSlashOffset+=1,src.length()));
	} else {
		return false;
	}
...
// подсчитать длину маски, необходимую для кол-ва устройств
FourOctets GetNeededMask(int numOfDevices) {
	int lenght = 0;
	while((2^lenght - 2) < numOfDevices) {
		lenght++;
	}
	return GetMaskBySize(lenght);
}

int main(void) {
	setlocale(LC_ALL, "RU");
	std::string inputString;
	SubnetWork networkList[9];
	SubnetWork temp;
	
	std::stringbuf strBuf1("10.17.0.0/22");
	std::stringbuf strBuf2("100");
	std::stringbuf strBuf3("20");

	try {
		// ip + маска
		getline(std::cin, inputString);
		if(SplitStrIpMask(inputString, temp.range[0], temp.mask, temp.maskLenght) == false) {
			throw UserException();
		}
		...
		for(int i=0; i<6; i++) {
			getline(std::cin, inputString);
			networkList[i].numOfDevices = atoi(inputString.c_str());
		}
		
		// seg Alpha, Beta, Gamma
		for (int i=6; i<9; i++)
		{
			networkList[i].numOfDevices = 2;
		}

		// считаем необходимые маски
		networkList[6].mask = GetNeededMask(networkList[6].numOfDevices);
